...
kemi<-bind_rows(trendvattendrag_kemi, flodmynningar_kemi)
  ...
  #                          `EU id`=="SE697145-157980"~"SE697231-157849", #Hornsjöbäcken3-153174", #Sörjabäcken
...
kemi %>% 
  dplyr::select(1:Provkommentar, 
         ...
         `Max provdjup (m)` ,
         `Vattentemperatur (°C)`) %>% 
  rename(`Vattentemperatur (°C) - helår`=`Vattentemperatur (°C)`)%>%
  as_tibble() %>% 
  ...
  gather(variable, value, `Vattentemperatur (°C) - helår`, na.rm = T) %>% 
...
vattenkemi_temp%>%
group_by(Övervakningsstation, `Nationellt övervakningsstations-ID`, `EU id`, `MS_CD C3`, Program, Delprogram, Projekt,`Stationskoordinat N/X`, `Stationskoordinat E/Y`, variable, Provtagningsår, Provtagningsmånad,  #månadsmedel
           n_years) %>% 
  summarise(value = mean(value, na.rm = T),
            n = n()) %>% 
  mutate(observed=1)%>%
  ungroup() %>%
  group_by(Övervakningsstation, `Nationellt övervakningsstations-ID`, `EU id`, `MS_CD C3`, Program, Delprogram, Projekt,`Stationskoordinat N/X`, `Stationskoordinat E/Y`, variable, Provtagningsår, n_years) %>%    #årsmedel
  summarise(value = mean(value, na.rm = T),
            n = sum(n),
            nobs=sum(observed))%>%
  filter(nobs>10)%>%
...
```{r}
trendvattendrag_kemi_data<- trendvattendrag_kemi_data1 %>% full_join(vattenkemi_temp3)%>%full_join(vattenkemi_temp2)
```
...
trendvattendrag_kemi_data_ny%>%
  arrange(Övervakningsstation,`Nationellt övervakningsstations-ID`,`EU id`, `MS_CD C3`, Program, Delprogram, Projekt, `Stationskoordinat N/X`, `Stationskoordinat E/Y`, variable, Provtagningsår) %>% 
...
```{r include=FALSE}
trendvattendrag_kemi_data %>% filter(variable !="Vattentemperatur (°C) - helår")%>%
 # filter(!(variable=="Si (mg/l)" & Provtagningsår<2008))%>%
  filter(n_years >= 10, !is.na(value)) %>% 
  group_by(Övervakningsstation, `Nationellt övervakningsstations-ID`, `EU id`, `MS_CD C3`, Program, Delprogram, Projekt,`Stationskoordinat N/X`, `Stationskoordinat E/Y`, variable, Provtagningsår, Provtagningsmånad,
           n_years) %>% 
  summarise(value = mean(value, na.rm = T)) %>% 
  ungroup() %>% 
  mutate(value=case_when(variable=="VJA"~-log10(value),  #beräkna pH ur vätejonaktivitet
         ...
         variable=case_when(variable=="VJA"~"pH",
                            TRUE~variable))%>%
  nest(data = c(-Övervakningsstation,-`Nationellt övervakningsstations-ID`, -`EU id`, -`MS_CD C3`,-variable, -Program, -Delprogram, -Projekt, -`Stationskoordinat N/X`, -`Stationskoordinat E/Y`)) %>% 
...
trendvattendrag_kemi_data %>% filter(variable !="Vattentemperatur (°C) - helår")%>%
 ...
 # filter(n_years >= 10, !is.na(value)) %>% 
  ...
  mutate(value=case_when(variable=="VJA"~-log10(value),  #beräkna pH ur vätejonaktivitet
         ...
         variable=case_when(variable=="VJA"~"pH",
                            TRUE~variable))%>%
  nest(data = c(-Övervakningsstation, -`EU id`, -`MS_CD C3`,-variable,-Program, -Delprogram, -Projekt, -`Stationskoordinat N/X`, -`Stationskoordinat E/Y`)) %>% 
  mutate(fit = map(data, possibly(~suppressMessages(rkt(date = as.data.frame(.x)$Provtagningsår,  # tar drygt 2 minuter
                                              block = as.data.frame(.x)$Provtagningsmånad,
                                       y=as.data.frame(.x)$value, correct = TRUE)), otherwise=NA_real_)),
         tidy = map(fit, possibly(~rkt_out(.x), otherwise=NA_real_))) %>% 
  ...
  mutate(sig.stars=cut(p.value, breaks=c(1, 0.05,0.01,0.001,0.0001,0), 
         ...
         tau = tau %>% round(4)) %>% 
         replace_na(list(sig.stars="****")) -> 
  vattendragkemi_mannkendall1_6år
```
...
trendvattendrag_kemi_data %>% filter(variable !="Vattentemperatur (°C) - helår")%>%
 ...
 # filter(n_years >= 10, !is.na(value)) %>% 
  ...
  mutate(sig.stars=cut(p.value, breaks=c(1, 0.05,0.01,0.001,0.0001,0), 
         ...
         tau = tau %>% round(4)) %>% 
         replace_na(list(sig.stars="****")) -> 
  vattendragkemi_mannkendall1_12år
```
...
vattenkemi_temp2 %>% 
 # filter(!(variable=="Si (mg/l)" & Provtagningsår<2008))%>%
  filter(n_years >= 10, !is.na(value)) %>% 
    filter(Provtagningsår>last_year-6)%>%
  group_by(Övervakningsstation, `Nationellt övervakningsstations-ID`,`EU id`, `MS_CD C3`, Program, Delprogram, Projekt,`Stationskoordinat N/X`, `Stationskoordinat E/Y`, variable, Provtagningsår,
           n_years) %>% 
  summarise(value = mean(value, na.rm = T)) %>% 
  ungroup() %>% 
  nest(data = c(-Övervakningsstation,-`Nationellt övervakningsstations-ID`, -`EU id`, -`MS_CD C3`,-variable,-Program, -Delprogram, -Projekt,  -`Stationskoordinat N/X`, -`Stationskoordinat E/Y`)) %>% 
  ...
  mutate(sig.stars=cut(p.value, breaks=c(1, 0.05,0.01,0.001,0.0001,0), 
         ...
         tau = tau %>% round(4)) %>% 
         replace_na(list(sig.stars="****")) -> 
  vattendragkemi_mannkendall_temp2_6år
```
```{r}
vattenkemi_temp2 %>% 
 # filter(!(variable=="Si (mg/l)" & Provtagningsår<2008))%>%
  ...
  ungroup() %>% 
  nest(data = c(-Övervakningsstation,-`Nationellt övervakningsstations-ID`, -`EU id`, -`MS_CD C3`,-variable,-Program, -Delprogram, -Projekt,  -`Stationskoordinat N/X`, -`Stationskoordinat E/Y`)) %>% 
  mutate(fit = map(data, possibly(~suppressMessages(rkt(date = as.data.frame(.x)$Provtagningsår, 
                                       y=as.data.frame(.x)$value, correct = TRUE)), otherwise=NA_real_)),
         tidy = map(fit, possibly(~rkt_out(.x), otherwise=NA_real_))) %>% 
  unnest(tidy) %>% 
  dplyr::select(-data, -fit) %>% 
  mutate(sig.stars=cut(p.value, breaks=c(1, 0.05,0.01,0.001,0.0001,0), 
         ...
         tau = tau %>% round(4)) %>% 
         replace_na(list(sig.stars="****")) -> 
  vattendragkemi_mannkendall_temp2_12år
```
```{r}
vattenkemi_temp2 %>% 
 # filter(!(variable=="Si (mg/l)" & Provtagningsår<2008))%>%
...
```{r}
vattendragkemi_mannkendall1%>%full_join(vattendragkemi_mannkendall_temp2)%>%mutate(Period="Hela perioden")->vattendragkemi_mannkendall_all
vattendragkemi_mannkendall1_6år%>%full_join(vattendragkemi_mannkendall_temp2_6år)%>%mutate(Period="6")->vattendragkemi_mannkendall_6år
vattendragkemi_mannkendall1_12år%>%full_join(vattendragkemi_mannkendall_temp2_12år)%>%mutate(Period="12")->vattendragkemi_mannkendall_12år


vattendragkemi_mannkendall_all%>%full_join(vattendragkemi_mannkendall_6år)%>%full_join(vattendragkemi_mannkendall_12år)->vattendrag_mannkendall
```
...
<!--   write_excel_csv("Outdata\\vattendrag_vattenkemi_mk.csv", na = "") -->
<!-- ``` -->
