...
pacman::p_load(tidyverse, readxl, lubridate, mgcv, magrittr, broom, forecast, zoo, rkt, DT, furrr, here, gratia)
...
out <-
  out %>% 
  mutate(across(all_of(c(numeric_var:ncol(out))),
            replace_less_than_var,
            bad_quality_na = bad_quality_na))
out <-
  out %>%
  mutate(across(all_of(c(1:(numeric_var - 1))),
                (function(x) {
                              x %>%
                                as.character() %>%
